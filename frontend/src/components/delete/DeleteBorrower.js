...
const DeleteBorrower = () => {
  ...
  const handleSearch = async (query) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URI}/api/borrowers/searchdel?query=${query}`);
  ...
  const handleDelete = async () => {
    if (confirmDelete === 'delete' && selectedBorrower) {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URI}/api/borrowers/${selectedBorrower._id}`, {
          method: 'DELETE',
          headers: {
