...
    if (!authorId) {
      // Nếu authorId không tìm thấy, nên dùng giá trị mặc định,
      // hoặc đảm bảo rằng form sẽ không hiển thị nếu không có authorId.
...
      bookId: book._id,
      title: book.title,
      authorName: book?.author?.authorName || '', // Đảm bảo an toàn
      category: book.category || '', 
      price: book.price ? book.price.toString() : '',
      preAuthorID: authorId || '' // <-- Đã thêm trường này
    });
  };
...
  // Đã xóa khối useEffect (vì nó không cần thiết và gây ra lỗi nếu không đầy đủ)
  
  return (
    <div className="p-14">
