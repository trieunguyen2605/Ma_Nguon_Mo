...
        const response = await fetch(`${process.env.REACT_APP_API_URI}/api/authors/search?query=${query}`);
...
  const handleSelectAuthor = (author) => { // Đã loại bỏ 'async' và 'try/catch'
    setSelectedAuthor(author);
    setShowAuthorList(false); // Ẩn danh sách sau khi chọn
    setFormData({
...
    e.preventDefault();
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URI}/api/authors/${selectedAuthor._id}`, {
        method: 'PUT',
        headers: {
...
            authorName: '',
            authorEmail: '',
            authorPhone: '' // Đảm bảo tất cả các trường được reset
        });
        setAuthors([]);
        setSelectedAuthor(null);
        setShowAuthorList(true); // Hiển thị lại danh sách/thanh tìm kiếm
      }
    } catch (error) {
...
  const handleReset = () => {
    setFormData({
      authorName: '', // <-- Đã sửa lỗi đánh máy
      authorEmail: '',
      authorPhone: ''
    });
    setSelectedAuthor(null);
...
        authorEmail: selectedAuthor.authorEmail || '', // <-- ĐÃ SỬA LỖI ĐÁNH MÁY
