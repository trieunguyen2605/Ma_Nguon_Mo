...
const ShowBorrowers = ({ type }) => {
  ...
  const handleSearch = useCallback(async (query) => {
    try {
      let response = null;
      if (type === 'all') {
        response = await fetch(`${process.env.REACT_APP_API_URI}/api/borrowers/search?query=${query}`);
      } else if (type === 'withoutbooks') {
        response = await fetch(`${process.env.REACT_APP_API_URI}/api/borrowers/searchdel?query=${query}`);
      }
